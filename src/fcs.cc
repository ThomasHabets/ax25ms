...
{
    const uint8_t fcs1 = data[data.size() - 2];
    const uint8_t fcs2 = data[data.size() - 1];
    const auto crc = ax25ms::fcs(data.substr(0, data.size() - 2));
    const uint16_t n = htons(fcs1 << 8 | fcs2); // TODO: what byte order to use?

    if (std::make_pair(fcs1, fcs2) != crc) {
        return { data, n, false };
    }
    return { data.substr(0, data.size() - 2), n, true };
}
